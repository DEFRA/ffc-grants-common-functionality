...
const getAnswersKeys = (questionKey, answersValues, ALL_QUESTIONS) => {
  const question = getQuestionByKey(questionKey, ALL_QUESTIONS)
  return answersValues.map(answerValue => {
    return question.answers.find(({ value }) => (value === answerValue)).key
  })
}
