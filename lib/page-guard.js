...
const guardDataCheck = (guardData, preValidationList, result, inverseResult, request, ALL_QUESTIONS) => {
  switch (guardData?.preValidationRule) {
    ...
    case 'NOT':
      ...
      preValidationList.forEach(preValidation => {
        // console.log('preValidation', preValidation.key)
        // console.log('preValidation', preValidation.values)
        if (!getYarValue(request, preValidation.key) ||
          preValidation.values.filter(answer => [getYarValue(request, preValidation.key)].flat().includes(getQuestionAnswer(preValidation.url, answer, ALL_QUESTIONS))).length === 0 ){
            // console.log(preValidation.values.filter(answer => [getYarValue(request, preValidation.key)].flat().includes(getQuestionAnswer(preValidation.url, answer, ALL_QUESTIONS))), 'RESULTTT')
          return true
        }else {
          return false
        }
